...
def send_emails():
    sender_email = EMAIL_USER or sender_entry.get()  # Default to entry if env variable is not set
    password = EMAIL_PASS or password_entry.get()
...
sender_entry.insert(0, EMAIL_USER or "")
sender_entry.grid(row=0, column=1)

tk.Label(root, text="Password:").grid(row=1, column=0, padx=10, pady=10)
password_entry = tk.Entry(root, width=40, show='*')
password_entry.insert(0, EMAIL_PASS or "")
